jQuery(document).ready(function($){ 'use strict';
    ...
    let font    = 'default',
        type    = 'default',
        weight  = 'default';
    let value   = $('.coblog-customizer-typography-input').val()
    if ( value ) {
        value   = value.split("#");
        ...
        if( font ){
            ...
            $('.coblog-field-typography-weight-field').html( (dataSelect[0].v).map( val => '<option '+(val==weight?' selected':'')+'>' + val + '</option>' ) );
        }
    }
    
    $('.coblog-field-typography-field').append( data.map( val => '<option'+(val.n==font?' selected':'')+'>' + val.n + '</option>' ) );
    
    $('.coblog-field-typography-field').on('change', function() {
        font = this.value
        dataSelect =  data.filter( val => val.n == font );
        type = dataSelect[0].f
        $(this).parents('.coblog-customizer-typography').find('.coblog-field-typography-weight-field').html( (dataSelect[0].v).map( val => '<option>' + val + '</option>' ) );
    });
    $('.coblog-field-typography-field, .coblog-field-typography-weight-field').on('change', function() {
        const selector = $(this).parents('.coblog-customizer-typography').find('.coblog-customizer-typography-input')
        selector.val( font + '#' + type + '#' + ($(this).parents('.coblog-customizer-typography').find('.coblog-field-typography-weight-field').val()||'default') )
